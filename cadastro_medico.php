...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
        ...
        try {
            $stmt = $pdo->prepare(
              "INSERT INTO medicos (nome, nome_mae, cpf, telefone, email, cep, rua, numero, bairro, cidade, estado, senha, cro, especialidade, data_nascimento) 
              VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
            );
            $stmt->execute([$nome, $nome_mae, $cpf, $telefone, $email, $cep, $rua, $numero, $bairro, $cidade, $estado, $senha_hash, $cro, $especialidade, $data_nascimento]);

            // seta sessão temporária para configurar 2FA imediatamente
            $_SESSION['pending_2fa'] = ['id' => $pdo->lastInsertId(), 'tipo' => 'medico'];
